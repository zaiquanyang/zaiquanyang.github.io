...
`CocoDetection`的初始化只用给出准备好了的COCO2017数据集的图像文件夹路径 **img_folder**， 比如训练集图像数据`/data/code/Det_Code/COCO_2017/train2017`,  以及注释文件路径 **ann_file**，如`/data/code/Det_Code/COCO_2017/annotations/instances_train2017.json`： **transforms**（指图像的增广变换函数），**return_masks**（对于检测涉及不到）。
