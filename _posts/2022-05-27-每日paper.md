...
- 完善评测代码后，验证：
  - 在使用数据增强 (RS-RC-CJ) 后的 Baseline 
    - TS-CAM-(RS-RC-CJ)-(BatchSize-64-2)   &#9745;
    - TS-CAM-(RS-RC-CJ)-(BatchSize-100-2)  &#9745;
  - (CCAM-Neg-Last2BLK-1.0T)的作用
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-50-2)   &#9745;
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-100-2), 之前跑过一次，这次是复现一下  &#9745;
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-80-2)  *06-25* &#9745;  
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-128-2)  *06-25* &#9745;  
      - **验证大的BatchSize是不是会出现GT_loc达到峰值再下降的原因。**  
     ---
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-100-2), 发现有点小差别。。。，Fuck, 再复现一次  &#9745;
  
  - (CCAM-Neg-Last2BLK-1.0T) 赋予不同的权重，**验证CCAM_loss的权重是不是会出现GT_loc达到峰值再下降的原因。**
    - GPTR-(CCAM-Neg-Last2BLK-1.0T-w-0.2)-(RS-RC-CJ)-(BatchSize-64-2)  *06-24* &#9745;
    - GPTR-(CCAM-Neg-Last2BLK-1.0T-w-0.6)-(RS-RC-CJ)-(BatchSize-100-2)  *06-24* &#9745;
    - GPTR-(CCAM-Neg-Last2BLK-1.0T-w-0.6)-(RS-RC-CJ)-(BatchSize-100-2)  *06-25* &#9745;
...
- [MaskViT: Masked Visual Pre-Training for Video Prediction](https://arxiv.org/pdf/2206.11894.pdf)
- [Entropy-driven Sampling and Training Scheme for Conditional Diffusion Generation](https://arxiv.org/pdf/2206.11474.pdf)
- [NeurIPS_2022_在投_Mutual Information-guided Knowledge Transfer for Novel Class Discovery](https://arxiv.org/abs/2206.12063)
- [IJCAI2021_Multi-view Feature Augmentation with Adaptive Class Activation Mapping](https://arxiv.org/pdf/2206.12943.pdf)
- [ICLR_2020_Self-labelling via simultaneous clustering and representation learning](https://github.com/yukimasano/self-label)