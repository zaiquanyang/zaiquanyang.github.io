...
- 完善评测代码后，验证：
  ...
  - (CCAM-Neg-Last2BLK-1.0T)的作用
    ...
    - GPTR-(CCAM-Neg-Last2BLK-1.0T)-(RS-RC-CJ)-(BatchSize-64-2), 之前跑过一次，这次是复现一下  &#9745;
    - 跑不同的batchsize是为了观察 (CCAM-Neg-Last2BLK-1.0T) 对模型定位和分类收敛的影响
  - (CCAM-Neg-Last2BLK-1.0T) 赋予不同的权重
